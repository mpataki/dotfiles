...
vim.cmd('SpelunkOpen')
local _, tab1_split = split_buf()
vim.cmd('tabnew')
local tab2_buf = vim.api.nvim_get_current_buf()
vim.cmd('SpelunkOpen')
P.ok(not vim.api.nvim_win_is_valid(tab1_split) and vim.api.nvim_get_current_buf() == tab2_buf,
  ':SpelunkOpen from another tab: closes the split in its own tab, leaves this tab\'s buffer')
vim.cmd('tabclose')
